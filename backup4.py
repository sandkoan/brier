...
def apply_partial(func: Callable, needs_interpreter: bool, *args: Any) -> Callable:
    def wrapper(interpreter: AIPLInterpreter | None, *remaining_args: Any) -> Any:
        if needs_interpreter:
            return func(interpreter, *args, *remaining_args)
        else:
            return func(*args, *remaining_args)
...
class AIPLInterpreter:
    ...
    def apply_operator(self, op_name: str, args: dict[str, Any], result: Any) -> Any:
        ...
        if op:
            ...
            if arity > len(args):
                return apply_partial(op, op.aipl_needs_interpreter, *args.values())
            else:
                if op.aipl_needs_interpreter:
                    return op(self, *args.values())
                else:
